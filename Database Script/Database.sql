...
VALUES  ( N'tthanh' , -- UserName - nvarchar(100)
          N'Nguyen Tien Thanh' , -- DisplayName - nvarchar(100)
          N'1' , -- PassWord - nvarchar(1000)
          0  -- Type - int
        )
GO
...
CREATE PROC SP_DeleteFoodAndBillInfoByCtgId
@CtgID INT 
AS
BEGIN
DECLARE @idfood INT
DECLARE @MyCursor1 CURSOR
SET @MyCursor1 = CURSOR FAST_FORWARD FOR 
SELECT id FROM dbo.Food WHERE idCategory = @CtgID

	OPEN @MyCursor1
	FETCH NEXT FROM @MyCursor1 INTO @idfood
	WHILE @@FETCH_STATUS = 0
		BEGIN 
	...
	CLOSE @MyCursor1
	DEALLOCATE @MyCursor1
	DELETE dbo.Food WHERE idCategory = @CtgID
